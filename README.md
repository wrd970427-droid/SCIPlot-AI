...
**为什么必须这样做？**  
很多小伙伴——尤其是医院、临床医疗工作者与医院科研人员——最担心的不是「图好不好看」，而是 **数据会不会泄露**：上传表格后，患者级别的检测值、临床字段、样本注释一旦进入公有云大模型，就可能触及患者隐私与院内合规红线。SCIPlot 因此把边界写死：LLM 只看到「你想画什么、列名叫什么、当前脚本怎么写」；**CSV/TSV 里的真实数值只在本地（或院内隔离环境）参与 R 出图**，不作为 Prompt 内容发给模型。这样既保留自然语言改图的便利，又尽量让医院场景敢用、能用。
