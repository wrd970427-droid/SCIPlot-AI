...
我们相信：未来的科研绘图助手不该是「会聊天的代码粘贴机」，而应是 **懂图类型、懂统计与版式边界、可审计、可回退** 的 Figure Engineer。
...
### 6. 隐私优先的设计约束
Prompt 侧仅允许：用户指令、列名、Spec 摘要、当前 R 源码。Catalog 中预留 `privacy.llm_allowed` / `local_only` 字段，面向后续院内部署场景。
